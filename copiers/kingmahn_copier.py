"""
copiers/kingmahn_copier.py - Kingmahn's Tribe (The Tribe Archives) Signal Copier
Direct Gold (XAUUSD) signal parsing with Split-Order partial profit-taking,
milestone profit trailing (+30/+50/+100/+150 pips), and channel Breakeven / Scale-Out commands.
"""
...
logger = logging.getLogger("KingmahnCopier")

KINGMAHN_DEFAULT_CHANNEL = -1003552447402
GOLD_ASSET_ID   = 74
GOLD_INSTRUMENT = "mcfd.74"

class KingmahnParser:
    REJECT_KEYWORDS = [
        "performance", "archives", "testimonials", "investment",
        "starting with the amount", "im only interested", "hello panel",
        "book your ea", "serious clients", "giveaway"
    ]
    ...
    def parse_signal(text: str) -> Optional[Dict[str, Any]]:
        ...
        # Reject pure commentary / promo chat
        if any(k in lower for k in KingmahnParser.REJECT_KEYWORDS) and not any(k in lower for k in ["buy gold", "sell gold", "gold buy", "gold sell"]):
            return None

        # Determine side
        side = None
        if re.search(r'\b(?:buy\s+gold|gold\s+buy)\b', lower) or (re.search(r'\bbuy\b', lower) and "gold" in lower):
            side = "BUY"
        elif re.search(r'\b(?:sell\s+gold|gold\s+sell)\b', lower) or (re.search(r'\bsell\b', lower) and "gold" in lower):
            side = "SELL"
        ...
        # Stop Loss
        sl_m = re.search(r'(?:stoploss|stop\s*loss|sl)[^\d\n\r]*([0-9]{4}(?:\.[0-9]+)?)', lower)
        if not sl_m:
            return None
        sl = float(sl_m.group(1))

        # Entry Range (@ 4316.00- 4323.00 or 4316 - 4323)
        ent_m = re.search(r'(?:@|entry|zone)?[^\d\n\r]*([0-9]{4}(?:\.[0-9]+)?)\s*[-\u2013\u2014]\s*([0-9]{4}(?:\.[0-9]+)?)', clean)
        emin, emax = None, None
        if ent_m:
            a, b = float(ent_m.group(1)), float(ent_m.group(2))
            emin, emax = min(a, b), max(a, b)

        # Take Profit levels (Take Profit 1: 4314.00, TP2: 4312.00, TP3: Open)
        tp1_m = re.search(r'(?:take\s*profit\s*1|tp\s*1)[^\d\n\r]*([0-9]{4}(?:\.[0-9]+)?)', lower)
        tp2_m = re.search(r'(?:take\s*profit\s*2|tp\s*2)[^\d\n\r]*([0-9]{4}(?:\.[0-9]+)?)', lower)
        tp3_m = re.search(r'(?:take\s*profit\s*3|tp\s*3)[^\d\n\r]*([0-9]{4}(?:\.[0-9]+)?)', lower)
        ...
        if not tp1:
            tp_all = re.findall(r'(?:take\s*profit|tp)[^\d\n\r]*([0-9]{4}(?:\.[0-9]+)?)', lower)
            if tp_all:
                tps = [float(x) for x in tp_all]
    ...
    def parse_instruction(text: str) -> Optional[Dict[str, Any]]:
        ...
        if any(k in lower for k in [
            "move sl to be", "set breakeven", "scale out 50%", "paid and protected",
            "switch to risk-free mode", "set stops to be", "secure your profits",
            "secure those gains", "hold with breakeven", "lock profit"
        ]):
            return {"type": "BREAKEVEN"}
        return None

class KingmahnCopier(BaseCopier):
    def __init__(self, mcp_client: IQForexMCPClient, channel_id: int = KINGMAHN_DEFAULT_CHANNEL,
                 lots: float = 1.0, leverage: int = 100, max_slippage: float = 4.0,
                 poll_interval: int = 15, enabled: bool = True):
        super().__init__("KingmahnTribe", channel_id, enabled)
        self.mcp = mcp_client
        self.lots = lots
    ...
    def set_lots(self, lots: float):
        ...
        logger.info(f"📊 [KingmahnTribe] Lot size set to: {self.lots}")

    def set_leverage(self, leverage: int):
        self.leverage = int(leverage)
        logger.info(f"⚡ [KingmahnTribe] Leverage set to: {self.leverage}x")
    ...
    async def execute_signal(self, sig: Dict[str, Any]):
        ...
        if exec_price <= 0:
            logger.error("❌ [KingmahnTribe] Could not fetch market price.")
            return
        ...
        try:
            ...
            if opp_pos:
                ...
                logger.warning(f"⚠️ [KingmahnTribe] Skipped {side} — An active {opp_side.upper()} position (#{pos_id}) already exists on Gold.")
                await self.notify(f"⚠️ [Kingmahn's Tribe] Skipped {side} — Opposing {opp_side.upper()} position #{pos_id} already active on Gold.")
                return
        except Exception as e:
            logger.warning(f"[KingmahnTribe] Error checking open positions: {e}")
        ...
        if emin and emax:
            if side == "BUY" and exec_price > (emax + self.max_slippage):
                logger.warning(f"⚠️ [KingmahnTribe] Market price {exec_price:.2f} slipped too far above entry max {emax:.2f}")
                await self.notify(f"⚠️ [Kingmahn's Tribe] Skipped BUY — Price {exec_price:.2f} slipped > {self.max_slippage} above max entry {emax:.2f}")
                return
            elif side == "SELL" and exec_price < (emin - self.max_slippage):
                logger.warning(f"⚠️ [KingmahnTribe] Market price {exec_price:.2f} slipped too far below entry min {emin:.2f}")
                await self.notify(f"⚠️ [Kingmahn's Tribe] Skipped SELL — Price {exec_price:.2f} slipped > {self.max_slippage} below min entry {emin:.2f}")
                return
        ...
        msg = (
            f"⚡ **[KINGMAHN SIGNAL] {side} @ ~{exec_price:.2f}**\n\n"
        ...
        for o in orders_to_place:
            ...
            if "order_id" in res:
                ...
                logger.info(f"✅ [KingmahnTribe] {o['tag']} placed! ID: #{order_id}")
                ...
                await self.notify(f"✅ **[KINGMAHN EXECUTED] #{order_id} {side} {o['lots']}L @ {exec_price:.2f}**")
                asyncio.create_task(self._monitor_position(order_id))
            else:
                logger.error(f"❌ [KingmahnTribe] {o['tag']} failed: {res}")
                await self.notify(f"❌ **[KINGMAHN FAILED] {o['tag']}**: {res.get('error', res)}")

    async def trigger_manual_breakeven(self, reason: str = "Channel Broadcast"):
        """Shifts all active Kingmahn positions to Breakeven."""
        if not self.open_positions:
            logger.debug("[KingmahnTribe] No active positions open to apply Breakeven. Ignoring.")
            return
        ...
        for order_id, pos in list(self.open_positions.items()):
            ...
            if not res.get("error"):
                ...
                await self.notify(
                    f"🛡️ **[KINGMAHN BREAKEVEN] #{pos_id} ({side})**\n\n"
                    f"• Trigger: {reason}\n"
                    f"• Trade is now Risk-Free! SL shifted to: `{be_level:.2f}`"
                )

    async def _monitor_position(self, order_id: int):
        """Monitors active Kingmahn trade with pip milestone scaling, breakeven, and profit trailing."""
        ...
        for _ in range(8):
            ...
            try:
                ...
                logger.warning(f"[KingmahnTribe] Position lookup error: {e}")
            if not pos.get("position_id"):
                await asyncio.sleep(4)

        if not resolved or not pos.get("position_id"):
            logger.warning(f"⚠️ [KingmahnTribe] Order #{order_id} failed to map to an active position. Pruning from tracking.")
        ...
        if pos_id:
            try:
                ...
                logger.warning(f"[KingmahnTribe] Notice setting post-fill SL/TP on #{pos_id}: {e}")

        logger.info(f"🛡️ [KingmahnTribe] Monitoring position #{pos_id or order_id} ({tag}) with milestone trailing & Breakeven.")
        ...
        while order_id in self.open_positions:
            ...
            try:
                ...
                logger.warning(f"[KingmahnTribe] Error listing positions: {e}")
                continue

            if not is_still_open:
                logger.info(f"📊 [KingmahnTribe] Position #{pos_id or order_id} ({tag}) closed! Resolving settlement...")
                ...
                if mid > 0:
                    ...
                    if gain_pips >= 30.0 and stage < 1:
                        ...
                        if not res.get("error"):
                            ...
                            logger.info(f"🛡️ [Kingmahn +30 Pips] Risk cut 50% on #{pos_id}! SL: {half_risk_sl}")
                            await self.notify(
                                f"🛡️ **[KINGMAHN DEFENSE +30 PIPS] #{pos_id} ({tag})**\n\n"
                    ...
                    if (gain_pips >= 50.0 or gain >= risk_dist) and stage < 2:
                        ...
                        if not res.get("error"):
                            ...
                            logger.info(f"🛡️ [Kingmahn +50 Pips / 1R] Breakeven activated on #{pos_id}! SL: {be_level}")
                            await self.notify(
                                f"🛡️ **[KINGMAHN BREAKEVEN +50 PIPS] #{pos_id} ({tag})**\n\n"
                                f"• Trade is now Risk-Free! SL shifted to: `{be_level:.2f}`"
                            )
                    ...
                    if gain_pips >= 100.0 and stage < 3:
                        ...
                        if not res.get("error"):
                            ...
                            logger.info(f"💰 [Kingmahn +100 Pips] Secured +50 Pips on #{pos_id}! SL: {lock_50}")
                            await self.notify(
                                f"🔒 **[KINGMAHN PROFIT LOCK +100 PIPS] #{pos_id} ({tag})**\n\n"
                    ...
                    if gain_pips >= 150.0 and stage < 4:
                        ...
                        if not res.get("error"):
                            ...
                            logger.info(f"💰 [Kingmahn +150 Pips] Secured +100 Pips on #{pos_id}! SL: {lock_100}")
                            await self.notify(
                                f"🔒 **[KINGMAHN PROFIT LOCK +150 PIPS] #{pos_id} ({tag})**\n\n"
                    ...
                    if gain_pips >= 200.0:
                        ...
                        if should_update:
                            ...
                            if not res.get("error"):
                                ...
                                logger.info(f"🎯 [Kingmahn Trailing Stop] Trailing SL updated to {trail_sl:.2f} on #{pos_id}")
    ...
    async def _log_trade_closure(self, order_id: int):
        ...
        try:
            ...
            logger.warning(f"[KingmahnTribe] Trade history lookup error: {e}")
        ...
        try:
            gsheet_logger.log_forex_margin_trade({
                ...
                "asset": f"Kingmahn Tribe Gold ({tag})",
                "side": side,
                "lots": lots,
                ...
                "risk_reward": "Kingmahn Tribe Signal",
                "exit_reason": reason,
                "position_id": pos_id or order_id,
                "balance_equity": eq
            })
        except Exception as e:
            logger.warning(f"[KingmahnTribe] GSheet log error: {e}")

        pnl_str = f"+${pnl:.2f}" if pnl >= 0 else f"-${abs(pnl):.2f}"
        if pnl > 0:
            header_line = f"🏆 **[KINGMAHN WON] {tag} {pnl_str}**"
        elif pnl == 0:
            header_line = f"🛡️ **[KINGMAHN BREAKEVEN] {tag} $0.00**"
        else:
            header_line = f"❌ **[KINGMAHN CLOSED] {tag} {pnl_str}**"
    ...
    async def handle_message(self, text: str, message_id: int, event: Any = None, msg_date: Any = None):
        ...
        if msg_date:
            try:
                ...
                logger.warning(f"[KingmahnTribe] Error checking message date: {e}")

        # 2. Check for Breakeven instructions
        inst = KingmahnParser.parse_instruction(text)
        if inst and inst["type"] == "BREAKEVEN":
            logger.info("📢 [KingmahnTribe] Received Secure Profit / Breakeven command from channel!")
            await self.trigger_manual_breakeven(reason="Channel Broadcast")
            return

        # 3. Check for trade signal
        sig = KingmahnParser.parse_signal(text)
        if sig and sig.get("type") == "SIGNAL":
            logger.info(f"🎯 [KingmahnTribe] New signal parsed: {sig['side']} | SL: {sig['sl']} | TP1: {sig['tp1']} | TP2: {sig['tp2']}")
