...
class KingmahnCopier(BaseCopier):
    ...
    async def execute_signal(self, sig: Dict[str, Any]):
        ...
        msg = (
            f"⚡ [Kingmahn's Tribe SIGNAL — SPLIT ENTRY]\n"
            f"Side : {side}\n"
        ...
        for o in orders_to_place:
            ...
            if "order_id" in res:
                ...
                await self.notify(f"✅ [Kingmahn's Tribe] {o['tag']} Live: #{order_id} {side} {o['lots']}L @ {exec_price:.2f}")
                asyncio.create_task(self._monitor_position(order_id))
            else:
                logger.error(f"❌ [KingmahnTribe] {o['tag']} failed: {res}")
                await self.notify(f"❌ [Kingmahn's Tribe] {o['tag']} Order Failed: {res.get('error', res)}")
    ...
    async def trigger_manual_breakeven(self, reason: str = "Channel Broadcast"):
        ...
        for order_id, pos in list(self.open_positions.items()):
            ...
            if not res.get("error"):
                ...
                await self.notify(
                    f"🛡️ [Kingmahn BREAKEVEN ACTIVATED]\n"
                    f"Trigger: {reason}\n"
                ...
                if mid > 0:
                    ...
                    if (gain_pips >= 50.0 or gain >= risk_dist) and stage < 2:
                        ...
                        if not res.get("error"):
                            ...
                            await self.notify(
                                f"🛡️ [Kingmahn BREAKEVEN +50 PIPS]\n"
                                f"Position #{pos_id} ({tag})\n"
                    ...
                    if gain_pips >= 100.0 and stage < 3:
                        ...
                        if not res.get("error"):
                            ...
                            await self.notify(
                                f"💰 [Kingmahn PROFIT LOCK +100 PIPS]\n"
                                f"Position #{pos_id} ({tag})\n"
                    ...
                    if gain_pips >= 150.0 and stage < 4:
                        ...
                        if not res.get("error"):
                            ...
                            await self.notify(
                                f"💰 [Kingmahn PROFIT LOCK +150 PIPS]\n"
                                f"Position #{pos_id} ({tag})\n"
