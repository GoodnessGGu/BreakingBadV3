...
class CallistoZoneParser:
    ...
    def parse_all(text: str) -> List[Dict[str, Any]]:
        """
        Parse ALL events from a message. Returns a list because one message
        can contain both an INVALIDATION and a new ZONE (e.g. CallistoFx
        NY Analysis Update style messages).
        """
        upper   = text.upper()
        results = []

        # Invalidations
        for inv_side in ("BUY", "SELL"):
            if f"{inv_side} ZONE INVALIDATED" in upper:
                results.append({"type": "INVALIDATE", "side": inv_side, "raw_text": text})

        # New zone(s) — match every occurrence
        # Handles: "BUY ZONE:", "SELL ZONE:", "NEW BUY ZONE:", "NEW SELL ZONE:"
    ...
    def parse_zone(text: str) -> Optional[Dict[str, Any]]:
        ...
        results = CallistoZoneParser.parse_all(text)
        ...
        for tf_sec in [60, 120, 300]:
            ...
            try:
                ...
                if o == 0 or c == 0:
                    continue
                if side == "BUY" and c > o:
                    logger.info(f"  Bullish {tf_label} candle confirmed: O={o:.2f} C={c:.2f}")
                    return True
                if side == "SELL" and c < o:
                    logger.info(f"  Bearish {tf_label} candle confirmed: O={o:.2f} C={c:.2f}")
                    return True
            except Exception as e:
        ...
        while True:
            try:
                ...
                async def handler(event):
                    ...
                    events_list = CallistoZoneParser.parse_all(text)
                    if not events_list:
                        logger.info("Could not parse any zone events from message.")
                        return

                    for ev in events_list:
                        if ev["type"] == "INVALIDATE":
                            logger.info(f"ZONE INVALIDATED: {ev['side']}")
